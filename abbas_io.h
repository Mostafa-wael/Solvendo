...
uint8_t abbas_digitalPinToBitMask(uint8_t pin)
{
    if (pin >= 0 && pin <= 7)
    {
        return _BV(pin);
...
uint8_t abbas_digitalPinToPort(uint8_t pin)
{
    if (pin >= 0 && pin <= 7)
    {
        return PRTD;
...
uint8_t *abbas_portModeRegister(uint8_t port)
{
    if (port == PRTB)
    {
...
uint8_t *abbas_portInputRegister(uint8_t port)
{
    if (port == PRTB)
    {
...
void abbas_pinMode(uint8_t pin, uint8_t mode)
{
    uint8_t bit = abbas_digitalPinToBitMask(pin);
    uint8_t port = abbas_digitalPinToPort(pin);
    volatile uint8_t *reg, *out;

    // JWS: can I let the optimizer do this?
    reg = abbas_portModeRegister(port);
...
uint8_t *abbas_portOutputRegister(uint8_t port)
{
    if (port == PRTB)
    {
...
void abbas_digitalWrite(uint8_t pin, uint8_t val)
{
    uint8_t bit = abbas_digitalPinToBitMask(pin);
    uint8_t port = abbas_digitalPinToPort(pin);
    volatile uint8_t *out;
    out = abbas_portOutputRegister(port);
    if (val == LOW)
    {
        *out &= ~bit;
    }
...
int abbas_digitalRead(uint8_t pin)
{
    uint8_t bit = abbas_digitalPinToBitMask(pin);
    uint8_t port = abbas_digitalPinToPort(pin);
    Serial.println(PIND);
    Serial.println(*abbas_portInputRegister(port));
    if (*abbas_portInputRegister(port) & bit)
        return HIGH;
    return LOW;
}

int abbas_analogRead(uint8_t ch)
{
    uint8_t pin = ch - 14;
    ADMUX = 1 << REFS0;
    ADCSRA |= (1 << ADEN);
...
uint8_t abbas_analogWrite(uint8_t apin, uint8_t cycle)
{
